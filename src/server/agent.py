...
class ReplayBuffer(object):
    ...
    def __init__(self, max_size, input_shape, n_actions):
        ...
        self.terminal_memory = np.zeros(self.mem_size,
                                        dtype=np.float32)  # float32 becasue tesorflow is perticular with data types, be aware of that.
...
class CriticNetwork(nn.Module):  # nn.Module give access to important methods, e.g. train, eval and parameters for updating the weights of the neural network.
    def __init__(self, beta, input_dims, fc1_dims, fc2_dims, n_actions, name, chkpt_dir='tmp/ddpg'):
        ...
        T.nn.init.uniform_(self.fc2.weight.data, -f2,
                           f2)  # the first parameter is the tensor you want to initialize, and then the lower and upper boundaries
...
class AgentDDPG(Agent):
    ...
    def choose_action(self, state) -> int:
        ...
        if not done:
            ...
            self.last_action_probs = self.compute_action(state)
